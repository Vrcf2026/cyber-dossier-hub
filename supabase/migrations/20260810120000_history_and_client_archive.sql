-- ============================================================
-- HISTÓRICO DE EDIÇÕES — guarda o estado anterior de uma secção
-- sempre que é alterada. Nunca se perde nada, mesmo que o texto
-- seja reescrito ou "restaurado" mais tarde (restaurar cria uma
-- nova entrada, a antiga fica sempre no histórico).
-- ============================================================
...
-- Só leitura para utilizadores aprovados. Nunca há INSERT/UPDATE/DELETE
-- direto por utilizadores — só o trigger (SECURITY DEFINER) escreve aqui,
-- o que torna o histórico à prova de alteração/apagamento manual.
...
AS $$
BEGIN
  -- só regista se algo relevante mudou de facto (evita ruído em updated_at-only)
...
-- ============================================================
-- ARQUIVO DE CLIENTES ENCERRADOS — bucket privado onde o PDF
-- completo gerado antes de eliminar um cliente fica guardado,
-- como segunda cópia de segurança (a primeira é o download
-- imediato no browser). Só admins conseguem ler.
-- ============================================================

INSERT INTO storage.buckets (id, name, public) VALUES ('client-archives', 'client-archives', false)
